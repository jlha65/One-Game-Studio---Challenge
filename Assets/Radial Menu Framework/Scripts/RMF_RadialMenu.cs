...
public class RMF_RadialMenu : MonoBehaviour {
    ...
    public List<RMFMenuSwitcher> menuSwitcher = new List<RMFMenuSwitcher>();

    [Tooltip("This is the element that is currently on top of the menu.")]
    private int m_currentTopElement = 0;
    public int currentTopElement
    {
        get { return m_currentTopElement; }
        set
        {
            m_currentTopElement = value;
            if (OnVariableChange != null)
                OnVariableChange(m_currentTopElement);
        }
    }
    ...
    void Update() {
        ...
        //Updates the topmost element of the menu
        for (int i = 0; i < elements.Count; i++)
        {
            //Debug.Log(elements[i].GetComponentInChildren<Button>().transform.position.y);
            if (elements[i].GetComponentInChildren<Button>().transform.position.y > elements[currentTopElement].GetComponentInChildren<Button>().transform.position.y)
            {
                currentTopElement = i;
            }
        }
        //Debug.Log(currentTopElement);
